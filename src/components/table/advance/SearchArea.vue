    ...
    <div class="search-area" ref="root">
        ...
        <div class="search-item" :key="index" v-for="(col, index) in searchCols">
            <div v-if="col.dataType === 'boolean'" :class="['title', {active: col.search.value !== undefined}]">
                ...
                <a-icon v-if="col.search.value !== undefined" class="close" @click="e => onCloseClick(e, col)"
            ...
            <div v-else-if="col.dataType === 'time'" :class="['title', {active: col.search.value}]">
                ...
                <slot v-else-if="col.slots && col.slots.title" :name="col.slots.title"></slot>
                <a-time-picker :format="col.search.format" v-model="col.search.value" placeholder="选择时间"
                    @change="(time, timeStr) => onCalendarChange(time, timeStr, col)"
                    @openChange="open => onCalendarOpenChange(open, col)" class="time-picker" size="small"
            ...
            <div v-else-if="col.dataType === 'date'" :class="['title', {active: col.search.value}]">
                ...
                <slot v-else-if="col.slots && col.slots.title" :name="col.slots.title"></slot>
                <a-date-picker :format="col.search.format" v-model="col.search.value" @change="onDateChange(col)"
                    class="date-picker" size="small" :getCalendarContainer="() => $refs.root" />
            </div>
            <div v-else-if="col.dataType === 'datetime'" class="title datetime active">
                ...
                <slot v-else-if="col.slots && col.slots.title" :name="col.slots.title"></slot>
                <a-date-picker :locale="locale" :format="col.search.format" v-model="col.search.value"
                    @change="(date, dateStr) => onCalendarChange(date, dateStr, col)"
                    @openChange="open => onCalendarOpenChange(open, col)" class="datetime-picker" size="small" show-time
                    :getCalendarContainer="() => $refs.root" />
            </div>
            <div v-else-if="col.dataType === 'datetimeRange'" class="title datetime active">
                <template v-if="col.title">
                    {{col.title}}
                </template>
                <slot v-else-if="col.slots && col.slots.title" :name="col.slots.title"></slot>
                <a-range-picker :locale="locale" :format="col.search.format" v-model="col.search.value"
                    @ok="(date, dateStr) => onCalendarChange(date, dateStr, col)"
                    @change="(date, dateStr) => onCalendarRangeChange(date, dateStr, col)" class="datetime-picker"
                    size="small" show-time :getCalendarContainer="() => $refs.root" />
            </div>
            <div v-else-if="col.dataType === 'select'" :class="['title', {active: col.search.value !== undefined}]">
                ...
                <slot v-else-if="col.slots && col.slots.title" :name="col.slots.title"></slot>
                <a-select :allowClear="true" :options="col.search.selectOptions" v-model="col.search.value" show-search
                    :filter-option="filterOption" placeholder="请选择..." @change="onSelectChange(col)" class="select"
                    slot="content" size="small" :get-popup-container="() => $refs.selectRoot">
                </a-select>
            </div>
            <div v-else :class="['title', {active: col.search.value}]">
                <a-popover @visibleChange="onVisibleChange(col, index)" v-model="col.search.visible" placement="bottom"
                    :trigger="['click']" :get-popup-container="() => $refs.root">
                    <template v-if="col.title">
                        {{col.title}}
                    </template>
                    <slot v-else-if="col.slots && col.slots.title" :name="col.slots.title"></slot>
                    <div class="value " v-if="col.search.value">:&nbsp;&nbsp;{{col.search.format && typeof
                        col.search.format === 'function' ? col.search.format(col.search.value) : col.search.value}}
                    </div>
                    ...
                    <div class="search-overlay" slot="title">
                        <a-input :id="`${searchIdPrefix}${index}`" :allow-clear="true" @keyup.esc="onCancel(col)"
                            @keyup.enter="onConfirm(col)" v-model="col.search.value" size="small" />
                    </div>
                </a-popover>
                <a-icon v-if="col.search.value" @click="e => onCloseClick(e, col)" class="close" type="close-circle"
                    theme="filled" />
            </div>
        </div>
    </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            onCalendarRangeChange(date, dateStr, col) {
                if (date.length == 0) {
                    const { momentEqual, backupAndEmitChange } = this
                    const { value, backup, format } = col.search
                    if (!col.search.visible && !momentEqual(value, backup, format)) {
            ...
            getFormat(col) {
                ...
                switch (dataType) {
                    ...
                    case 'datetimeRage': return 'YYYY-MM-DD HH:mm:ss'
            ...
            getConditions(columns) {
                const conditions = {}
                columns.filter(item => item.search.value !== undefined && item.search.value !== '' && item.search.value !== null && item.search.value.length != 0)
                    .forEach(col => {
            ...
            momentEqual(target, source, format) {
                if (target === source) {
                    return true
                } else if (!Array.isArray(target) && target && source && target.format(format) === source.format(format)) {
                    return true
                }
                return false
            },
