<template>
    <div>
        <a-modal title="监控" width="600px" :visible="visible" :confirm-loading="saving" @ok="onSubmit"
            @cancel="handleCancel">
            <a-form @submit="onSubmit" :form="form" :colon="false" :layout="formLayout">
                ...
                <a-row :gutter="{ md: 40 }">
                    ...
                    <!-- 代收用户不显示 -->
                    <a-col :md="{ span: 24 }" v-show="user_role!='collect'">
                        <a-form-item label="用户">
                            <a-select allowClear showSearch placeholder="请选择 用户" v-decorator="decorator.user_id">
    ...
    export default {
        ...
        created() {
            ...
            if (this.user_role == 'collect') {
                this.decorator.user_id[1].rules[0].required = false;
            } else {
                this.decorator.user_id[1].rules[0].required = true;
            }
            this.getUsers()
