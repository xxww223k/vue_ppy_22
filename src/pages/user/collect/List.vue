<template>
    <div>
        <advance-table row-key="id" :defaultExpandAllRows="true" :data-source="dataSource" :columns="columns"
            ...
            <!-- 信用 -->
            <template slot="format_credit" slot-scope="row">
                <a-tag color="#f50">{{ row.record.format_credit }}</a-tag>
            </template>
            <!-- 费率 -->
...
<script>
    ...
    const columns = [
        ...
        {
            dataIndex: "format_credit",
            key: "format_credit",
            title: "信用",
            scopedSlots: { customRender: "format_credit" },
        },
        {
            dataIndex: "status",
            key: "status",
    ...
    export default {
        ...
        methods: {
            ...
            get() {
                this.loading = true
                const query = {
                    search: { ...this.conditions, role: 'collect' },
