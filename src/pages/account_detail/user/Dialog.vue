<template>
    <div>
        <a-modal title="账变" :visible="visible" :confirm-loading="confirmLoading" @ok="onSubmit" @cancel="handleCancel">
                ...
                <a-row :gutter="{ md: 40 }">
                    <a-col :md="{ span: 24 }">
                        <a-form-item label="账目类型 ">
                            <a-select placeholder="请选择 账目类型" v-decorator="decorator.type" disabled>
...
<script>
    ...
    import { USER as URL, ACCOUNT_DETAIL } from "@/services/api";
        ...
        methods: {
            showModal() {
                ...
                this.$nextTick(() => {
                    ...
                    this.form.setFieldsValue({ type: 5 })
                })
            },
            handleOk() {
            ...
            getUser() {
                ...
                request(URL, METHOD.GET, query).then((res) => {
                    ...
                    for (const item of list.data) {
                        if (item.role.slug == "collect" || item.role.slug == "client") {
            ...
            onSubmit(e) {
                ...
                this.form.validateFields((err) => {
                    if (!err) {
                        ...
                        request(ACCOUNT_DETAIL, method, fields).then(this.afterSubmit).catch(() => {
                            this.$message.error('系统出错了~')
                        }).finally(() => {
                            this.saving = false
                        })
